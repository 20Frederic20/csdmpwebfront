...
const createInsuranceSchema = z.object({
  ...
  insurance_id: z.string().min(1, 'Veuillez sélectionner une assurance'),
  policy_number: z.string().min(1, 'Le numéro de police est requis'),
  priority: z.number().min(1).max(10),
  is_active: z.boolean(),
});

type CreateInsuranceFormValues = z.infer<typeof createInsuranceSchema>;
...
export function CreatePatientInsuranceModal({ isOpen, onClose, onSuccess, patientId }: CreatePatientInsuranceModalProps) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreateInsuranceFormValues>({
  ...
  useEffect(() => {
    if (patientId) {
      reset(prev => ({ ...prev, patient_id: patientId }));
    }
  }, [patientId, reset]);
  ...
  const onSubmit = async (data: CreateInsuranceFormValues) => {
    try {
      ...
      {loadingInsurances ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            />
            {errors.insurance_id && <p className="text-sm text-red-500">{errors.insurance_id.message}</p>}
          </div>
